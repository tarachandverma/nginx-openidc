...
oidc_authz_sub_request_type oidc_authz_subrequest_type_authorize =
	...
	#define ngx_http_authz_oidc_rprintf(r,arg1) \
	...
	#define ngx_http_authz_oidc_rprintf1(r,tem,arg1) \
	...
	#define ngx_http_authz_oidc_rprintf2(r,tem,arg1,arg2) \
	...
	#define ngx_http_authz_oidc_rprintf3(r,tem,arg1,arg2,arg3) \
	...
	#define ngx_http_authz_oidc_rprintf4(r,tem,arg1,arg2,arg3,arg4) \
	...
	#define ngx_http_authz_oidc_rprintf5(r,tem,arg1,arg2,arg3,arg4,arg5) \
	...
	#define ngx_http_authz_oidc_rputs(str,r) \
...
typedef struct {
    ...
    Config* config;
} ngx_http_authz_oidc_srv_conf_t;
...
} ngx_http_authz_oidc_subrequest_ctx_t;

static void *ngx_http_authz_oidc_create_srv_conf(ngx_conf_t *cf);
static char *ngx_http_authz_oidc_merge_srv_conf(ngx_conf_t *cf,
    void *parent, void *child);
static ngx_int_t ngx_http_authz_oidc_post_config(ngx_conf_t *cf);
static ngx_int_t ngx_http_authz_oidc_checkForAccess(ngx_http_request_t * httpRequest);
static ngx_int_t ngx_http_authz_oidc_handler(ngx_http_request_t *r);
static void ngx_http_authz_oidc_setActionHeaders(ngx_http_authz_oidc_request_t*r, apr_table_t* headers, array_header* actionHeaders, template_engine* tengine, char* originUri, int addHeaderToHttpRequest);
static int ngx_http_authz_oidc_execUri(ngx_http_authz_oidc_request_t* r, char* uri,int isForward, int isPermanent, char* originUri);
static char* ngx_http_authz_oidc_getPostEntity(ngx_http_authz_oidc_request_t* r, long* sz);
static void ngx_http_authz_oidc_deleteHeader(ngx_http_request_t *r, u_char *key);
static ngx_int_t ngx_http_authz_oidc_setHeader(ngx_http_request_t *r, const char *key, const char *val);
static ngx_int_t ngx_http_authz_oidc_deleteHeaderPart(ngx_list_t *l, ngx_list_part_t *cur, ngx_uint_t i);
static int oidc_index(ngx_http_authz_oidc_request_t *r, Config* config);
static int oidc_version(ngx_http_authz_oidc_request_t *r, Config* config);
static int oidc_rewrite_actionmappings(ngx_http_authz_oidc_request_t *r, Config *config);
static int oidc_rewrite_match(ngx_http_authz_oidc_request_t *r, Config *config);
static int oidc_rewrite_pageactions(ngx_http_authz_oidc_request_t *r, Config *config);
static int oidc_config_core_status(ngx_http_authz_oidc_request_t *r, Config *config);
static int oidc_headers(ngx_http_authz_oidc_request_t *r, Config *config);
static char* ngx_http_authz_oidc_getFullRequestUrl(ngx_http_authz_oidc_request_t* r);
static ngx_int_t ngx_http_authz_oidc_subrequest_post_handler(ngx_http_request_t *r, void *data, ngx_int_t rc);
static int ngx_http_authz_oidc_set_id_token_header(ngx_http_request_t* r);
static ngx_int_t ngx_http_authz_oidc_set_subrequest_post_body(ngx_http_request_t * r, ngx_http_request_t * sr, const char* requestBody, ngx_str_t contentType);
static ngx_int_t ngx_http_authz_oidc_preAuthorize(ngx_http_authz_oidc_request_t* r, Config* config);
static ngx_int_t ngx_http_authz_oidc_create_post_subrequest(ngx_http_request_t * r, oidc_authz_sub_request_type sub_request_type, const char* requestBody);

ngx_str_t  ngx_http_authz_oidc_content_length_header_key =
        ngx_string("Content-Length");
ngx_str_t  ngx_http_authz_oidc_content_type_header_key =
        ngx_string("Content-Type");
ngx_str_t  ngx_http_authz_oidc_content_type_formurlencoded =
        ngx_string("application/x-www-form-urlencoded");
ngx_str_t  ngx_http_authz_oidc_content_type_json =
        ngx_string("application/json");

static void ngx_http_authz_oidc_terminate() {
	if(mainPool) apr_pool_destroy(mainPool);
   apr_terminate();
}

static int ngx_http_authz_oidc_die(int exitCode, const char *message, apr_status_t reason) {
    char msgbuf[80];
...
static void ngx_http_authz_oidc_initializeAPR() {
	apr_status_t status;
	// Initialize library
	status = apr_initialize();
	if(status!=APR_SUCCESS) { ngx_http_authz_oidc_die(-2, "Could not initialize", status); }
	atexit(ngx_http_authz_oidc_terminate);
	
	// create mainPool
	status = apr_pool_create(&mainPool, NULL);
	if(status!=APR_SUCCESS) { ngx_http_authz_oidc_die(-2, "Could not initialize main pool", status); }
	
	initializedAPR = 1;
}

static char *
ngx_http_authz_oidc_set_home_dir(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_home_dir:%d", getpid());

    return NGX_CONF_OK;
}

static char *
ngx_http_authz_oidc_set_log_file(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_log_file:%d", getpid());

    return NGX_CONF_OK;
}

static char *
ngx_http_authz_oidc_set_shm(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_shm:%d", getpid());

    return NGX_CONF_OK;
}

static char *
ngx_http_authz_oidc_set_remote_path(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_remote_path:%d", getpid());

    return NGX_CONF_OK;
}

static char *
ngx_http_authz_oidc_set_pass_phrase(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_pass_phrase:%d", getpid());

    return NGX_CONF_OK;
}

static char *
ngx_http_authz_oidc_set_header_prefix(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_header_prefix:%d", getpid());
...
static char* ngx_http_authz_oidc_postRefreshBind(apr_pool_t* p,Config* config){
...
static void ngx_http_authz_oidc_postConfigStarting(apr_pool_t* p, const char* defn_name, int is_virtual, Config* config){
	...
	if(config->oidcConfigFile!=NULL){
		...
		if(error==NULL){
			...
			error=ngx_http_authz_oidc_postRefreshBind(p,config);
...
static char *
ngx_http_authz_oidc_set_configcore_file(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_authz_oidc_srv_conf_t *sscf = conf;
...
//	printf("ngx_http_authz_oidc_set_configcore_file:%d", getpid());
	if(!initializedAPR) {
		ngx_http_authz_oidc_initializeAPR();
	}
...
//        return NGX_ERROR;
	...
	ngx_http_authz_oidc_postConfigStarting(mainPool,"authz_oidc",FALSE,sscf->config);

    return NGX_CONF_OK;
}
static ngx_command_t  ngx_http_authz_oidc_commands[] = {
    { ngx_string("AUTHZOIDC_HomeDir"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
	  ngx_http_authz_oidc_set_home_dir,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL }, 
    { ngx_string("AUTHZOIDC_LogFile"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
	  ngx_http_authz_oidc_set_log_file,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL },
	{ ngx_string("AUTHZOIDC_SharedMemory"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
	  ngx_http_authz_oidc_set_shm,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL },
	{ ngx_string("AUTHZOIDC_RemotePath"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE123,
	  ngx_http_authz_oidc_set_remote_path,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL },
	{ ngx_string("AUTHZOIDC_PassPhrase"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
	  ngx_http_authz_oidc_set_pass_phrase,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL },
	{ ngx_string("AUTHZOIDC_HeaderPrefix"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
	  ngx_http_authz_oidc_set_header_prefix,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL },
    { ngx_string("AUTHZOIDC_ConfigFile"),
	  NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE12,
	  ngx_http_authz_oidc_set_configcore_file,
	  NGX_HTTP_SRV_CONF_OFFSET,
	  0,
	  NULL },	  
      ngx_null_command
...
static ngx_http_module_t  ngx_http_authz_oidc_module_ctx = {
    NULL,            						/* preconfiguration */
    ngx_http_authz_oidc_post_config,            /* postconfiguration */

    NULL,                                  /* create main configuration */
    NULL,                                  /* init main configuration */

    ngx_http_authz_oidc_create_srv_conf,          /* create server configuration */
    ngx_http_authz_oidc_merge_srv_conf,           /* merge server configuration */
...
ngx_module_t  ngx_http_authz_oidc_module = {
    NGX_MODULE_V1,
    &ngx_http_authz_oidc_module_ctx,              /* module context */
    ngx_http_authz_oidc_commands,                 /* module directives */
...
static void *
ngx_http_authz_oidc_create_srv_conf(ngx_conf_t *cf)
{
	ngx_http_authz_oidc_srv_conf_t  *sscf;

    sscf = ngx_pcalloc(cf->pool, sizeof(ngx_http_authz_oidc_srv_conf_t));
    if (sscf == NULL) {
        return NULL;
    }
//    printf("ngx_http_authz_oidc_create_srv_conf:%p %d\r\n", sscf, getpid());
    return sscf;
}


static char *
ngx_http_authz_oidc_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
	ngx_http_authz_oidc_srv_conf_t *prev = parent;
	ngx_http_authz_oidc_srv_conf_t *conf = child;
...
//    printf("ngx_http_authz_oidc_merge_srv_conf:%p %p %d\r\n", prev,conf,getpid());
    return NGX_CONF_OK;
}

static ngx_http_authz_oidc_request_t* ngx_http_authz_oidc_createRequest(apr_pool_t* p) {
	ngx_http_authz_oidc_request_t* r = (ngx_http_authz_oidc_request_t *) apr_pcalloc(p, sizeof(ngx_http_authz_oidc_request_t));
    r->pool = p;
...
static void ngx_http_authz_oidc_setHeadersIn(ngx_http_request_t *httpRequest, ngx_http_authz_oidc_request_t* r, char* headerPrefix) {
    ...
    for (i = 0; /* void */ ; i++) {
		...
		if ( prefixLen>0 && prefixLen < h[i].key.len && (ngx_strcasestrn(h[i].key.data, headerPrefix, prefixLen-1)==h[i].key.data) ) {
			...
			/*
			  ...
			  h[i].value.data = NULL;
			*/
			ngx_http_authz_oidc_deleteHeaderPart(&httpRequest->headers_in.headers, part, i);
		}else{
...
ngx_int_t ngx_http_authz_oidc_setHeaderInHeadersOut(ngx_http_request_t *r, const char *key, const char *val) {
    ngx_table_elt_t   *h;
...
ngx_int_t ngx_http_authz_oidc_setHeaderInHeadersIn(ngx_http_request_t *r, const char *keyStr, const char *valueStr) {
    ngx_table_elt_t   *h;
...
static ngx_int_t ngx_http_authz_oidc_post_config(ngx_conf_t *cf) {
    ...
    *h = ngx_http_authz_oidc_checkForAccess;
...
//    *h = ngx_http_authz_oidc_handler;

//    printf("ngx_http_authz_oidc_post_config:%d\r\n", getpid());
    return NGX_OK;
}

static int ngx_http_authz_oidc_addResponseHeaderCallback(void *rec, const char *keyStr, const char *valueStr){
	ngx_http_request_t *httpRequest = (ngx_http_request_t*)rec;
	if(keyStr&&valueStr&&httpRequest) {
	    ngx_http_authz_oidc_setHeaderInHeadersOut(httpRequest, keyStr, valueStr);
	}
	return 1;
}

static ngx_int_t ngx_http_authz_oidc_sendResponse(ngx_http_authz_oidc_request_t* r) {
...
ngx_table_elt_t* ngx_http_authz_oidc_lookupHeader(ngx_http_request_t *r, u_char *key) {
    ngx_list_part_t *part;
    ngx_table_elt_t *h;
    ngx_uint_t i;
...
ngx_int_t ngx_http_authz_oidc_setHeader(ngx_http_request_t *r, const char *key, const char *val) {
    ngx_table_elt_t   *h;
 ...
 * delete header part
 */
static ngx_int_t ngx_http_authz_oidc_deleteHeaderPart(ngx_list_t *l,
                               ngx_list_part_t *cur,
                               ngx_uint_t i) {
 ...
 */
void ngx_http_authz_oidc_deleteHeader(ngx_http_request_t *r, u_char *key) {
    ngx_list_part_t *part;
    ngx_table_elt_t *h;
    ngx_uint_t i;
    size_t len = strlen((char *)key);
    ...
    for (i = 0; ; i++) {
        ...
        if (len == h[i].key.len && ngx_strcasecmp(key, h[i].key.data) == 0) {
            ...
            /*
              ...
              h[i].value.data = NULL;
            */
        	ngx_http_authz_oidc_deleteHeaderPart(&r->headers_in.headers, part, i);
        }
    }
}

static const char *ngx_http_authz_oidc_requestVar(ngx_http_authz_oidc_request_t *r, const char* arg)
{
	char* start = NULL;
...
static const char *ngx_http_authz_oidc_serverVars(ngx_http_authz_oidc_request_t *r, const char* arg)
{
	 return (arg!=NULL) ? apr_hash_get(r->variableHash, arg, APR_HASH_KEY_STRING) : NULL;
}
static const char *ngx_http_authz_oidc_requestCookie(ngx_http_authz_oidc_request_t *r, const char *arg)
{
	if(arg==NULL) return "(null)";
...
static const char *ngx_http_authz_oidc_urlDecodeToken(ngx_http_authz_oidc_request_t *r, const char *arg){
	return url_decode2(r->pool,(char*)arg);
}

static const char *ngx_http_authz_oidc_urlEncodeToken(ngx_http_authz_oidc_request_t *r, const char *arg){
	return url_encode2(r->pool,(char*)arg);
}

	typedef struct tag_handler{
		const char c;
		const char* (*func)(ngx_http_authz_oidc_request_t*,const char* arg);
	}tag_handler;
	
	static const tag_handler tagHandlers[] = {
			{'r', ngx_http_authz_oidc_requestVar},
			{'s', ngx_http_authz_oidc_serverVars},
			{'c', ngx_http_authz_oidc_requestCookie},
			{'u', ngx_http_authz_oidc_urlDecodeToken},
			{'U', ngx_http_authz_oidc_urlEncodeToken}
	};

static const char* ngx_http_authz_oidc_getTagValue(char* s, ngx_http_authz_oidc_request_t*r, char**next) {
	unsigned int i;
...
static char* ngx_http_authz_oidc_parseString(pool*p, char*src, ngx_http_authz_oidc_request_t *r){
	...
	while(*src!='\0'){
		if(*src=='%'&&(str = ngx_http_authz_oidc_getTagValue(src, r, &src))){
			strcpy(q, str);
			q += strlen(str);
		}else{
			*q++=*src++;
...
static char* ngx_http_authz_oidc_processTags(action_header *hdr, ngx_http_authz_oidc_request_t *r, template_engine* tengine, char* originUri)
{
   char* hdrValue = ngx_http_authz_oidc_parseString(r->pool, apr_pstrdup(r->pool, hdr->value), r);
   ...
   char* headerValue=NULL;		
		...
		for(i=0; i<actionHeaders->nelts; i++){
			...
			if(hdr!=NULL){
				switch (hdr->action) {
				case header_add:
					headerValue = ngx_http_authz_oidc_processTags(hdr, r, tengine, originUri);
					apr_table_add(headers, hdr->name, headerValue);
					if(addHeaderToHttpRequest){
						ngx_http_authz_oidc_setHeader(r->httpRequest, hdr->name, headerValue);
					}
					break;
				case header_append:
					headerValue = ngx_http_authz_oidc_processTags(hdr, r, tengine, originUri);
...
//						}
//					} else {
//						char *new_val = ngx_http_authz_oidc_processTags(hdr, r, tengine, originUri);
...
//					break;
				case header_set:
		            if (!strcasecmp(hdr->name, "Content-Type")) {
		            	char* content_type = ngx_http_authz_oidc_processTags(hdr, r, tengine, originUri);
				...
				case header_unset:
					...
					if(addHeaderToHttpRequest){
						ngx_http_authz_oidc_deleteHeader(r->httpRequest,hdr->name);
					}
					break;
				case header_merge:
				case header_echo:
...
static int ngx_http_authz_oidc_execCustomResponse(ngx_http_authz_oidc_request_t *r, int responseCode, char* contentType, const char* responseBodyTemplate, template_engine* tengine) {

		if(responseBodyTemplate!=NULL){

			// process template
			char* responseBody = ngx_http_authz_oidc_parseString(r->pool, apr_pstrdup(r->pool, responseBodyTemplate), r);
			/**
			 * Set the headers in response stream.
			 */
			apr_table_do(ngx_http_authz_oidc_addResponseHeaderCallback, r->httpRequest, r->headers_out, NULL);
...
static int ngx_http_authz_oidc_execPageAction(ngx_http_authz_oidc_request_t *r, page_action* paction, char* originUri){
	char* destUri=NULL;
	array_header* matches=NULL;
	int ret=0;

//		AP_LOG_ERROR(r,"ngx_http_authz_oidc_execPageAction");
	if(paction!=NULL){
		...
		if (paction->requestHeaders!=NULL) {
			ngx_http_authz_oidc_setActionHeaders(r, r->headers_in, paction->requestHeaders, paction->templateEngineRef, originUri, TRUE);
		}			
...
//				AP_LOG_ERROR1(r,"paction->handler_internal=%s",paction->handler_internal);
		...
		if(paction->uri!=NULL){
			...
			if(paction->advancedTemplate==TRUE){
				destUri = ngx_http_authz_oidc_parseString(r->pool, apr_pstrdup(r->pool, destUri), r);
			}

			if (paction->responseHeaders!=NULL) {
				ngx_http_authz_oidc_setActionHeaders(r, r->headers_out, paction->responseHeaders, paction->templateEngineRef, originUri, FALSE);
			}
			ret = ngx_http_authz_oidc_execUri(r,destUri,paction->isForward,paction->isPermanent,originUri);
			return ret;
			...
			if(paction->response!=NULL) {
				return ngx_http_authz_oidc_execCustomResponse(r, paction->response->code, paction->response->contentType, paction->response->body, paction->templateEngineRef);
			}
		}
	}
	return NGX_DECLINED;
...
int ngx_http_authz_oidc_execUri(ngx_http_authz_oidc_request_t* r, char* uri,int isForward, int isPermanent, char* originUri) {
	if(uri!=NULL){
		//execute
		if(isForward){
			if(originUri!=NULL){
				ngx_http_authz_oidc_setHeader(r->httpRequest, "X-MODAUTH-ORIGIN-URI", originUri);
			}
	//		if(r->filename!=NULL){
...
static ngx_int_t ngx_http_authz_oidc_getPostEntityINT(ngx_http_request_t *r) {
    ngx_int_t                   rc;
...
static char* ngx_http_authz_oidc_getPostEntity(ngx_http_authz_oidc_request_t* r, long* sz) {
	...
	ngx_http_authz_oidc_getPostEntityINT(r->httpRequest);
...
static char* ngx_http_authz_oidc_getFullRequestUrl(ngx_http_authz_oidc_request_t *req){
...
typedef int (*ngx_http_authz_oidc_handler_func)(ngx_http_authz_oidc_request_t*r, Config* config);

typedef struct ngx_http_authz_oidc_handler_t{
	ngx_http_authz_oidc_handler_func handlerFunc;
}ngx_http_authz_oidc_handler_t;

static const ngx_http_authz_oidc_handler_t ngx_http_oidcHandlers[] = {
		{oidc_index},
		{oidc_version},
		{oidc_rewrite_match},
...
void ngx_http_authz_oidc_addHeaderWithPrefix(ngx_http_authz_oidc_request_t* r,
	...
	if(headerPrefix!=NULL) {
		...
		if(addHeaderToHttpRequest) {
			ngx_http_authz_oidc_setHeader(r->httpRequest, headerName, value);
		}
	}
}

void ngx_http_authz_oidc_processState(ngx_http_authz_oidc_request_t* r) {
	uri_components *url = &r->parsed_uri;
...
ngx_http_authz_oidc_hash_str(u_char *src, size_t n)
{
    ngx_uint_t  key;
...
#define ngx_http_authz_oidc_hash_literal(s)                                        \
    ngx_http_authz_oidc_hash_str((u_char *) s, sizeof(s) - 1)


static ngx_int_t
ngx_http_authz_oidc_set_content_headers(ngx_http_request_t *r, off_t contentLength, ngx_str_t contentType)
{
    ...
    h->key = ngx_http_authz_oidc_content_length_header_key;
    ...
    h->hash = ngx_http_authz_oidc_hash_literal("content-length");
    ...
    h->key = ngx_http_authz_oidc_content_type_header_key;
    h->lowcase_key = ngx_pnalloc(r->pool, h->key.len);
    if (h->lowcase_key == NULL) {
        return NGX_ERROR;
    }
    ...
    h->value = contentType;
    h->hash = ngx_http_authz_oidc_hash_literal("content-type");
    r->headers_in.content_type = h;

    return NGX_OK;
}

int ngx_http_authz_oidc_processRequest(ngx_http_authz_oidc_request_t* r, config_core* 	configCore,
	...
	if(id_token!=NULL) {
		...
		if(jwt==NULL||jwt->claim==NULL) {
			ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_STATUS_HEADER, "failure");
			ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_DESC_HEADER, "oidc_parsing_failed");
		...
		// set session state
		ngx_http_authz_oidc_processState(r);
	}
	...
	if(claimObj==NULL&&configCore->cipherConfig!=NULL&&configCore->cipherConfig->crypto_passphrase!=NULL) { // check for cookie
		...
		if(oidc_session!=NULL) {
			...
			if(status>0){
				...
				ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_STATUS_HEADER, "failure");
				ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_DESC_HEADER, "oidc_session_expired");
	...
	if(claimObj!=NULL) {
		if(claimObj->expiry>apr_time_sec(apr_time_now())) {
			ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_STATUS_HEADER, "success");
		...
		// set headers
			if(claimObj->issuer!=NULL) {
				...
				if(oauthConfig->oidcProvider->issuer!=NULL&&(strcmp(claimObj->issuer,oauthConfig->oidcProvider->issuer)!=0)) {
					ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_STATUS_HEADER, "failure");
					ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_DESC_HEADER, "oidc_issuer_mismatch");
					apr_table_add(r->headers_out, "X-OIDC-ERROR", "oidc_issuer_mismatch");
					return NGX_DECLINED;
				}
				ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_ISSUER_HEADER, claimObj->issuer);
			}
			if(claimObj->subject!=NULL) {
				ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_SUBJECT_HEADER, claimObj->subject);
			}
			if(claimObj->audience!=NULL) {
				ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_AUDIENCE_HEADER, claimObj->audience);
			}
			if(claimObj->roles!=NULL) {
				char* rolesCsv = cu_serializeCsvFromStringArray(r->pool, claimObj->roles);
				if(rolesCsv!=NULL) {
					ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_ROLES_HEADER, rolesCsv);
				}
			}
			if(claimObj->options!=NULL) {
				...
				if(oidcNonce!=NULL&&rpSessionID!=NULL&&(strcmp(oidcNonce,rpSessionID)!=0)) {
					ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_STATUS_HEADER, "failure");
					ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_DESC_HEADER, "oidc_nonce_mismatch");
				...
				for (hi = apr_hash_first(r->pool, claimObj->options); hi; hi = apr_hash_next(hi)) {
					...
					ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, name, value);
				}
			}
		}else{
			ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_STATUS_HEADER, "failure");
			ngx_http_authz_oidc_addHeaderWithPrefix(r, headerPrefix, addHeaderToHttpRequest, OIDC_DESC_HEADER, "oidc_claim_expired");
...
static ngx_int_t ngx_http_authz_oidc_checkForAccess(ngx_http_request_t *httpRequest) {
    ...
    ngx_http_authz_oidc_srv_conf_t* conf =(ngx_http_authz_oidc_srv_conf_t*) ngx_http_get_module_srv_conf(httpRequest, ngx_http_authz_oidc_module);
	...
	if(cfg_syncSelf(p,conf->config->configCore)>0){
		ngx_http_authz_oidc_postRefreshBind(p,conf->config);
	}

	//set method type
	ngx_http_authz_oidc_deleteHeader(httpRequest, (u_char *)"X-REQUEST-METHOD");
	char* method = apr_pstrndup(p, (char*)httpRequest->method_name.data, httpRequest->method_name.len);
	ngx_http_authz_oidc_setHeader(httpRequest, "X-REQUEST-METHOD", method);
...
#endif
	ngx_http_authz_oidc_deleteHeader(httpRequest, (u_char *)"X-REQUEST-SCHEME");
	ngx_http_authz_oidc_setHeader(httpRequest, "X-REQUEST-SCHEME", (isHttps) ? "https" : "http");

	// create the request.
	ngx_http_authz_oidc_request_t* r = ngx_http_authz_oidc_createRequest(p);
	...
	// set headers_in
	ngx_http_authz_oidc_setHeadersIn(httpRequest, r, conf->config->configCore->oidcHeaderPrefix);
    ...
    // run preAuth
	int retCode = ngx_http_authz_oidc_preAuthorize(r, conf->config);
	...
	ngx_http_authz_oidc_setHeader(r->httpRequest, OIDC_RP_SESSIONID, rpSessionID);

	// Authorization phase
	retCode = ngx_http_authz_oidc_processRequest(r, conf->config->configCore, conf->config->oidcConfig, TRUE, conf->config->configCore->oidcHeaderPrefix);
	if(retCode!=NGX_DECLINED) {
		apr_table_do(ngx_http_authz_oidc_addResponseHeaderCallback, httpRequest, r->headers_out, NULL);
		retCode = ngx_http_authz_oidc_sendResponse(r);
		apr_pool_destroy(p);
		return retCode;
	}
	...
	if(pathmapping!=NULL){
		...
		char* originUri=ngx_http_authz_oidc_getFullRequestUrl(r);
		retCode = ngx_http_authz_oidc_execPageAction(r, action,originUri);
		apr_table_do(ngx_http_authz_oidc_addResponseHeaderCallback, httpRequest, r->headers_out, NULL);
		if(action!=NULL&&action->handler_internal!=NULL) {
			r->handler = apr_pstrdup(r->pool, action->handler_internal);
			int i;
			int numHandlers=sizeof(ngx_http_oidcHandlers)/sizeof(ngx_http_authz_oidc_handler_t);
...
static ngx_int_t ngx_http_authz_oidc_handler(ngx_http_request_t * httpRequest) {
	char* hostname;
    pool* p;
	...
	if(cfg_syncSelf(r->pool,conf->config->configCore)>0){
		ngx_http_authz_oidc_postRefreshBind(r->pool,conf->config);
	}
...
int oidc_index(ngx_http_authz_oidc_request_t *r, Config* config) {

	if (strcmp(r->handler, "oidc_index")) {
		return NGX_DECLINED;
	}

	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rprintf(r,"<HEAD><TITLE>Tool Index</TITLE></HEAD>\n");
	ngx_http_authz_oidc_rputs("<BODY>\n",r);
	ngx_http_authz_oidc_rprintf(r,"<LEFT><H1>Tool Index</H1></LEFT><BR>\n");
	ngx_http_authz_oidc_rputs("<TABLE align='left'><tr>",r);
	ngx_http_authz_oidc_rputs("<td valign='top'><table><tr><td><b><font color='green'>Handlers</font></b></td></tr>",r);
	ngx_http_authz_oidc_rputs("<TR><TD><a href='/oidc/version'>Version Details</a></TD></TR>",r);
    ngx_http_authz_oidc_rputs("<TR><TD><a href='/oidc/config-status'>ConfigCore Details</a></TD></TR>",r);
	ngx_http_authz_oidc_rputs("<TR><TD><a href='/oidc/rewrite-pageactions'>PostAuth Actions Details</a></TD></TR>",r);
	ngx_http_authz_oidc_rputs("<TR><TD><a href='/oidc/rewrite-actionmappings'>OIDC Config Details</a></TD></TR>",r);
    ngx_http_authz_oidc_rputs("<TR><TD><a href='/oidc/headers'>Show Headers</a></TD></TR>",r);
	ngx_http_authz_oidc_rputs("</table>",r);
	ngx_http_authz_oidc_rputs("</td>",r);

	ngx_http_authz_oidc_rputs("</table>",r);

	ngx_http_authz_oidc_rputs("</td>",r);

	ngx_http_authz_oidc_rputs("</tr></TABLE>",r);

	ngx_http_authz_oidc_rputs("</BODY></HTML>",r);
    return NGX_OK;
}

int oidc_version(ngx_http_authz_oidc_request_t *r, Config* config){
	...
	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rprintf(r,"<HEAD><TITLE>Version Info</TITLE></HEAD>\n");
	ngx_http_authz_oidc_rputs("<BODY>\n",r);
	
	ngx_http_authz_oidc_rputs("<TABLE>",r);
	ngx_http_authz_oidc_rprintf(r,"<TR><TD colspan='2'><b>Version Info</b></td></tr>\n");
	ngx_http_authz_oidc_rprintf1(r,"<TR><TD>Module: </td><td>%s</td></tr>\n",VERSION_ID);
	ngx_http_authz_oidc_rprintf1(r,"<TR><TD>LibCurl: </td><td>%s</td></tr>\n",hc_getInfo(r->pool));
	ngx_http_authz_oidc_rprintf1(r,"<TR><TD>RewriteCore: </td><td>PCRE/%s</td></tr>\n",rc_getInfo(r->pool));
	ngx_http_authz_oidc_rprintf1(r,"<TR><TD>Server: </td><td>%s</td></tr>\n",NGINX_VER);
	ngx_http_authz_oidc_rputs("</TABLE>",r);
	return NGX_OK;
}
 ...
 * Rewrite Handlers
 */
static void rewrite_pageaction_display(ngx_http_authz_oidc_request_t *r,page_action* pa,match_list* ml){
	if(pa!=NULL){
		ngx_http_authz_oidc_rprintf1(r,"<td>%s",pa->id);
		if(ml&&ml->name!=NULL){
			ngx_http_authz_oidc_rprintf1(r,"<br/>MatchList:%s",ml->name);
		}
		ngx_http_authz_oidc_rprintf(r,"</td>");
		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",pa->handler_internal);
		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",SAFESTR(pa->uri));
		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",SAFESTR(pa->regex));
		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",pa->isForward!=1?"false":"true");
		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",pa->isPermanent!=1?"false":"true");
		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",pa->description);
	}else{
		ngx_http_authz_oidc_rputs("<td colspan='5'>&nbsp</td>",r);
	}
}
...
void authzoidc_displayActionHeader(ngx_http_authz_oidc_request_t*r, action_header* hdr) {
	
	if(hdr==NULL) return;
	
	if(hdr->action==header_unset) {
		ngx_http_authz_oidc_rprintf2(r,"<tr><td>&nbsp;&nbsp;</td><td><li>%s&nbsp;{%s}</td></tr>",
				authzoidc_getActionStr(hdr->action),hdr->name);
	}
	else {
		if(hdr->regex!=NULL) {
			ngx_http_authz_oidc_rprintf4(r,"<tr><td>&nbsp;&nbsp;</td><td><li>%s&nbsp;{%s:%s}&nbsp;regex:%s</td></tr>",
					authzoidc_getActionStr(hdr->action),hdr->name, hdr->value, hdr->regex);
		}else{
			ngx_http_authz_oidc_rprintf3(r,"<tr><td>&nbsp;&nbsp;</td><td><li>%s&nbsp;{%s:%s}</td></tr>",
					authzoidc_getActionStr(hdr->action),hdr->name, hdr->value);
		}
	}
}

static void rewrite_pageaction_displayAll(ngx_http_authz_oidc_request_t *r,page_action* pa){
	int i=0;
	array_header* arry=NULL;
	if(pa!=NULL){
		ngx_http_authz_oidc_rputs("<table>",r);
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td><strong>%s</strong>",SAFESTR(pa->id));
		if(pa->isDebug==1){ngx_http_authz_oidc_rprintf(r," <font color='maroon'>DEBUG</font> ");}
		if(pa->description!=NULL){ngx_http_authz_oidc_rprintf1(r," [%s]</td></tr>",pa->description);}else{ngx_http_authz_oidc_rprintf(r,"</td></tr>");}
		
		if(pa->regex!=NULL){ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>Regex: %s</td></tr>",SAFESTR(pa->regex));}
		if(pa->handler_internal!=NULL){ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>Handler: %s</td></tr>",pa->handler_internal);}
		if(pa->response!=NULL){ngx_http_authz_oidc_rprintf3(r,"<tr><td>&nbsp;</td><td>Response[%d:%s]: <xmp>%s</xmp></td></tr>",pa->response->code, pa->response->contentType, pa->response->body);}
		if(pa->uri!=NULL) { ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>Uri:%s</td></tr>",pa->uri); }
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>isForward: %s</td></tr>",pa->isForward!=1?"false":"true");
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>isPermanent: %s</td></tr>",pa->isPermanent!=1?"false":"true");
		if(pa->advancedTemplate==TRUE){
			ngx_http_authz_oidc_rprintf(r,"<tr><td>&nbsp;</td><td>advancedTemplate: true</td></tr>");
		}
		if(pa->requestHeaders!=NULL){
			ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>RequestHeaders[%d]</td></tr>",pa->requestHeaders->nelts);
		...
		if(pa->responseHeaders!=NULL){
			ngx_http_authz_oidc_rprintf1(r,"<tr><td>&nbsp;</td><td>ResponseHeaders[%d]</td></tr>",pa->responseHeaders->nelts);
		...
		ngx_http_authz_oidc_rputs("NULL",r);
	}
	
}
	
static void authzoidc_errorMessage(ngx_http_authz_oidc_request_t *r){
	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rprintf(r,"<HEAD><TITLE>Rewrite Page Actions - NULL</TITLE></HEAD>\n");
	ngx_http_authz_oidc_rprintf(r,"<body>ActionMappings are NULL</body>\n");
	ngx_http_authz_oidc_rprintf(r,"</html>\n");
}

static void authzoidc_pathmappings_displayMatchList(ngx_http_authz_oidc_request_t *r,array_header* arr){
	int i=0;
	match_list* list=NULL;
	if(arr!=NULL&&arr->nelts>0){
		ngx_http_authz_oidc_rprintf(r,"<br/>MatchLists:");
		for(i=0;i<arr->nelts;i++){
			list=(match_list*)cu_getElement(arr,i);
			if(i!=0){ngx_http_authz_oidc_rprintf(r,",");}
			ngx_http_authz_oidc_rprintf1(r,"%s",list->name);
		}
	}
}

static void authzoidc_pathmappings_display(ngx_http_authz_oidc_request_t *r,array_header* arr,char* name){
	int x=0;
	path_mapping* pmap=NULL;
	int i=0;
	pathmapping_action* pmaction=NULL;
	
	ngx_http_authz_oidc_rprintf2(r,"<br><b><font color='blue'>Path Mappings - %s (%d)</font></b>",name,arr->nelts);
	if(name,arr->nelts>0){
		ngx_http_authz_oidc_rputs("<TABLE border='1'>",r);
		ngx_http_authz_oidc_rputs("<tr><td>Path Regex</td><td>ID</td><td>handler</td><td>uri</td><td>Regex</td><td>isForward</td><td>isPermanent</td><td>Description</td></tr>",r);
			...
			if(pmap->pmactions!=NULL&&pmap->pmactions->nelts>0){
				for(i=0;i<pmap->pmactions->nelts;i++){
					if ( pmap->ignoreCase==TRUE ) {
						ngx_http_authz_oidc_rprintf1(r,"<tr><td><b>%s</b><br>IgnoreCase=true</br>",pmap->pathRegex);
					}else{
						ngx_http_authz_oidc_rprintf1(r,"<tr><td><b>%s</b>",pmap->pathRegex);
					}
					...
					ngx_http_authz_oidc_rputs("</td></tr>",r);
				}
			}
		}
		ngx_http_authz_oidc_rputs("</TABLE>",r);
...
static void authzoidc_pathmapping_display_withnote(ngx_http_authz_oidc_request_t *r,path_mapping* pathMap,page_action*action,char* note, apr_time_t startTime){
		
		if(pathMap!=NULL){
				ngx_http_authz_oidc_rprintf2(r,"<tr><td>%s</td><td>%d us</td>",note, apr_time_now()-startTime);
				ngx_http_authz_oidc_rprintf1(r,"<td><font color='darkgreen'>%s</font>",pathMap->pathRegex);
				authzoidc_pathmappings_displayMatchList(r,pathMap->matchLists);
				ngx_http_authz_oidc_rprintf(r,"</td>");
				rewrite_pageaction_display(r,action,NULL);
		}else{
				ngx_http_authz_oidc_rprintf1(r,"<tr><td>%s</td><td>(Default)</td>",note);
				rewrite_pageaction_display(r,NULL,NULL);
		}
		ngx_http_authz_oidc_rputs("</tr>",r);
}


static int authzoidc_printTableCallBack(void *rec, const char *key, const char *value){
		ngx_http_authz_oidc_request_t *r=(ngx_http_authz_oidc_request_t*)rec;
		if(strstr(key,"Cookie")==0) {
			ngx_http_authz_oidc_rprintf2(r,"<tr><td>%s = </td><td>%s</td></tr>\r\n",key,value);
		}
		return 1;
	}
static void authzoidc_printTable(ngx_http_authz_oidc_request_t *r,apr_table_t* table){
	if(table!=NULL&&!apr_is_empty_table(table)){
		ngx_http_authz_oidc_rputs("<TABLE><tr><td valign='top'>Table Headers:</td><td><table>",r);
		apr_table_do(authzoidc_printTableCallBack,(void*)r,table,NULL);
		ngx_http_authz_oidc_rputs("</table></td></tr></TABLE>",r);
	}
}
	
static int authzoidc_pathMatchSection(ngx_http_authz_oidc_request_t *r, Config* config){
	...
	ngx_http_authz_oidc_rputs("Check Path Mapping:<br>",r);
	ngx_http_authz_oidc_rputs("<form><table>",r);
	ngx_http_authz_oidc_rprintf1(r,"<tr><td>IP:</td><td><input type='text' name='ipParam' size='30'  value='%s'></td></tr>\r\n",SAFESTRBLANK(ipParam));
	ngx_http_authz_oidc_rprintf1(r,"<tr><td>Path:</td><td><input type='text' name='pathParam' size='80' value='%s'/></td></tr>",SAFESTRBLANK(pathParam));
	ngx_http_authz_oidc_rprintf1(r,"<tr><td valign='top'>HeadersIn:</td><td><textarea name='headersInParam' cols='60' rows='4'>%s</textarea></td></tr>\r\n",SAFESTRBLANK(headersInParam));
	ngx_http_authz_oidc_rputs("<tr><td colspan='2'><INPUT type='submit' value='Match' /></td></tr>\n",r);
	ngx_http_authz_oidc_rputs("</table></form>\n",r);
	if(pathParam!=NULL){
		...
		ngx_http_authz_oidc_rputs("<TABLE border='1' cellspacing='0' bordercolor='blue'>",r);
...
static void authzoidc_printMatchListMatch(ngx_http_authz_oidc_request_t *r,match_list_match* match){
	int i=0;
	//match_list_match_nvp* nvp=NULL;
	match_list_header* hdr=NULL;
	match_list_env* env=NULL;
	if(match==NULL) return;
	ngx_http_authz_oidc_rprintf(r,"<table><tr><td valign='top'><li/></td>");
	if(match->cascade==FALSE){
		ngx_http_authz_oidc_rprintf(r,"<td valign='top'>Cascade:</td><td align='left'>false</td></tr><tr><td>&nbsp;</td>");
	}		
	if(match->host!=NULL){
		ngx_http_authz_oidc_rprintf1(r,"<td valign='top'>Host:</td><td align='left'>%s</td></tr><tr><td>&nbsp;</td>", match->host);
	}		
	if(match->ip!=NULL){
		ngx_http_authz_oidc_rprintf1(r,"<td valign='top'>IP:</td><td align='left'>%s</td></tr><tr><td>&nbsp;</td>",
				authzoidc_getMatchIpDetails(r->pool,match->ip));
	}
	if(match->path!=NULL){
		ngx_http_authz_oidc_rprintf1(r,"<td valign='top'>Path:</td><td align='left'>%s</td></tr><tr><td>&nbsp;</td>",
				authzoidc_getMatchPathDetails(r->pool,match->path));
	}
	if(match->event!=NULL){
		ngx_http_authz_oidc_rprintf3(r, "<td valign='top'>Event:</td><td valign='top'><font color='%s'><li/></td><td><table><tr><td><i/>Start:</td><td>%s</td><td><i/>End:</td><td>%s</td></font></tr></tr></table></td></tr><td>&nbsp;</td>",
	...
	if(match->headerList!=NULL&&match->headerList->nelts>0){
		ngx_http_authz_oidc_rprintf(r,"<td valign='top'>Headers:</td><td>");
		ngx_http_authz_oidc_rprintf(r,"<table>");
		for(i=0;i<match->headerList->nelts;i++){
			hdr=(match_list_header*)cu_getElement(match->headerList,i);
			ngx_http_authz_oidc_rprintf1(r,"<tr><td valign='top'>%s</td></tr>",authzoidc_getHeaderDetails(r->pool,hdr));
		}
		ngx_http_authz_oidc_rprintf(r,"</table>");
		ngx_http_authz_oidc_rprintf(r,"</td></tr>");
	}
	ngx_http_authz_oidc_rprintf(r,"</table>");
}
static void authzoidc_matchLists(ngx_http_authz_oidc_request_t *r,oidc_config* actm){
	match_list* list=NULL;
	...
	ngx_http_authz_oidc_rprintf1(r,"<tr><td colspan='4'><font color='maroon'><strong>MatchLists [%d]:</strong></font></td></tr>",cnt);
	if(cnt>0){
		for (hi = shapr_hash_first(r->pool,actm->match_lists); hi; hi = shapr_hash_next(hi)) {
	           shapr_hash_this(hi, NULL, NULL, &val);
	     	   if(val!=NULL){
	     		  list=(match_list*)val;
	     		  ngx_http_authz_oidc_rprintf1(r,"<tr><td width='15'>&nbsp;</td><td>&nbsp;</td><td valign='top'>%s</td><td>",list->name);
	     		  
	     		  ngx_http_authz_oidc_rputs("<table>",r);
	     		  for(i=0;i<list->list->nelts;i++){
	     			  match=(match_list_match*)cu_getElement(list->list,i);
	     			  ngx_http_authz_oidc_rputs("<tr><td>",r);
...
static void authzoidc_displayTemplateEngine(ngx_http_authz_oidc_request_t *r,template_engine* tengine){
	shapr_hash_index_t* hi;
	template_eng_livetemplate* lt=NULL;
	void* val, * key;
	
	ngx_http_authz_oidc_rputs("<TABLE>",r);
	ngx_http_authz_oidc_rprintf1(r,"<tr><td>Template Engine:</td><td>%s</td></tr>",tengine!=NULL?"VALID":"FAIL");
	if(tengine!=NULL){
		
		ngx_http_authz_oidc_rputs("<tr><td colspan='2'><TABLE>",r);
		ngx_http_authz_oidc_rprintf(r,"<tr><td>Token:</td><td> Description</td><td> Test Result</td></tr>");
		for(hi=shapr_hash_first(r->pool,tengine->templateHash);hi;hi=shapr_hash_next(hi)){
			...
			if(lt->engineTemplate!=NULL){
				ngx_http_authz_oidc_rprintf2(r,"<tr><td><li/> %s:</td><td> %s</td></tr>",lt->engineTemplate->id,lt->engineTemplate->description);
			}else{
				ngx_http_authz_oidc_rprintf(r,"<tr><td colspan='3'><li/> NULL Template</td></tr>");
			}
		}
		ngx_http_authz_oidc_rputs("<TABLE></td></tr>",r);
		
	}
	ngx_http_authz_oidc_rputs("<TABLE>",r);
}

  int oidc_rewrite_actionmappings(ngx_http_authz_oidc_request_t *r, Config *config) {
	...
	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rprintf(r,"<HEAD><TITLE>OIDC Config</TITLE></HEAD>\n");
	ngx_http_authz_oidc_rputs("<BODY>\n",r);

	if(actm->oidcProvider!=NULL) {
		ngx_http_authz_oidc_rprintf(r,"<TABLE><tr><td><font color='black'><strong>OIDCProvider</strong></font></td></tr>");
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>MetadataUrl</td><td>%s</td></tr>",actm->oidcProvider->metadataUrl);
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>Issuer</td><td>%s</td></tr>",actm->oidcProvider->issuer);
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>AuthorizationEndpoint</td><td>%s</td></tr>",actm->oidcProvider->authorizationEndpoint);
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>JwksUri</td><td>%s</td></tr></TABLE>",actm->oidcProvider->jwksUri);
	}

	ngx_http_authz_oidc_rprintf(r,"<TABLE><tr><td><font color='black'><strong>Session</strong></font></td></tr>");
	ngx_http_authz_oidc_rprintf2(r,"<tr><td>RelyingParty</td><td>name=%s</td><td>expiry(days)=%d</td></tr>",actm->rpSession->name, actm->rpSession->age);
	ngx_http_authz_oidc_rprintf2(r,"<tr><td>OIDCProvider</td><td>name=%s</td><td>expiry(days)=%d</td></tr></TABLE>",actm->oidcSession->name, actm->oidcSession->age);
...
//	authzoidc_printTable(r,r->headers_in);
	...
	ngx_http_authz_oidc_rputs("</BODY>",r);
	ngx_http_authz_oidc_rputs("</HTML>\n",r);

	return NGX_OK;
}
 
  int oidc_rewrite_pageactions(ngx_http_authz_oidc_request_t *r, Config *config) {
	...
	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rputs("<HEAD><TITLE>Rewrite Page Actions</TITLE></HEAD>\n",r);
	ngx_http_authz_oidc_rputs("<BODY>\n",r);
	
	ngx_http_authz_oidc_rputs("<TABLE border='0'>",r);
	ngx_http_authz_oidc_rputs("<tr><td>&nbsp</td><td>",r);
	for (hi = shapr_hash_first(r->pool,actm->page_actions); hi; hi = shapr_hash_next(hi)) {
           shapr_hash_this(hi, &key, NULL, &val);
     	   if(key!=NULL&&val!=NULL){
     	   		...
     	   		ngx_http_authz_oidc_rputs("<br>",r);
//	        		ngx_http_authz_oidc_rprintf1(r,"<td>%s</td>",key);
        	}
	}
	ngx_http_authz_oidc_rputs("</td></tr></TABLE>",r);
 ...
 int oidc_rewrite_match(ngx_http_authz_oidc_request_t *r, Config *config) {
	...
	//display
	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rputs("<HEAD><TITLE>Rewrite Match Testing</TITLE></HEAD>\n",r);
	ngx_http_authz_oidc_rputs("<BODY>\n",r);
	ngx_http_authz_oidc_rputs("<TABLE>",r);
	ngx_http_authz_oidc_rprintf1(r,"<tr><td>RewriteCore: </td><td>%s</td></tr>",rc_getInfo(r->pool));
	ngx_http_authz_oidc_rputs("<form>",r);
	ngx_http_authz_oidc_rputs("<tr><td>Value(Url): </td><td>",r);
	ngx_http_authz_oidc_rprintf1(r,"<input type='text' value='%s' name='valueParam' size='180'/>\r\n",
			(valueParam!=NULL) ? cu_nonHtmlToHtmlChar(r->pool, valueParam) : SAFESTRBLANK(valueParam));
	ngx_http_authz_oidc_rputs("</td></tr>\r\n",r);
	ngx_http_authz_oidc_rputs("<tr><td>Regex: </td><td>",r);
	ngx_http_authz_oidc_rprintf1(r,"<input type='text' value='%s' name='regexParam' size='180'/>\r\n",
			(regexParam!=NULL) ? cu_nonHtmlToHtmlChar(r->pool, regexParam) : SAFESTRBLANK(regexParam));
	ngx_http_authz_oidc_rputs("</td></tr>\r\n",r);
	ngx_http_authz_oidc_rputs("<tr><td>Template: </td><td>",r);
	ngx_http_authz_oidc_rprintf1(r,"<input type='text' value='%s' name='templateParam' size='180'/>\r\n",SAFESTRBLANK(templateParam));
	ngx_http_authz_oidc_rputs("</td></tr>\r\n",r);
	ngx_http_authz_oidc_rputs("<tr><td></td><td><INPUT type='submit' value='go!' /></td></tr>\r\n</form>",r);
	...
	if(regexParam!=NULL&&valueParam!=NULL){
		...
		ngx_http_authz_oidc_rprintf1(r,"<tr><td>Result: (%ld usecs)</td><td>",startTime);
		if(rcoreRet!=NULL){
			ngx_http_authz_oidc_rprintf1(r,"FAILURE: %s</td></tr>",rcoreRet);
		}else{
			ngx_http_authz_oidc_rputs("SUCCESS</td></tr>",r);
		}
		if(templateParam!=NULL){
			ngx_http_authz_oidc_rprintf1(r,"<tr><td></strong>Template Engine String</strong>:</td><td>%s</td></tr>",SAFESTR(te_templateString(r->pool,config->oidcConfig->templateEngine,templateParam,pmatches)));
		}
		
		if(pmatches!=NULL){
			
			ngx_http_authz_oidc_rprintf1(r,"<tr><td colspan='2'></strong>Found Matches (%d)</strong></td></tr>",pmatches->nelts);
			
			if(pmatches->nelts>0){
				ngx_http_authz_oidc_rprintf(r,"<tr><td>&nbsp</td><td><table>");
				for(i=0;i<pmatches->nelts;i++){
					elt=cu_getElement(pmatches,i);
					ngx_http_authz_oidc_rprintf2(r,"<tr><td><strong>$%d: </strong></td><td>%s</td></tr>",i,
							cu_nonHtmlToHtmlChar(r->pool, elt));
				}
				ngx_http_authz_oidc_rprintf(r,"</table></td></tr>");
			}
		}
	}
 ...
 static int oidc_section_SharedHeap(const char* title, ngx_http_authz_oidc_request_t *r,shared_heap* sheap){
 	shared_page* pg;
 	segment_header* seg;
 	int x;
 	if(sheap!=NULL){
 		ngx_http_authz_oidc_rprintf1(r,"\n<TR><TD colspan='2'><b>Shared Heap Page - %s </b></TD></TR>",title);
 		ngx_http_authz_oidc_rprintf1(r,"<TR><TD>TimeStamp: </TD><TD>%s</TD></TR>",ctime(&(sheap->timestamp)));
 		ngx_http_authz_oidc_rprintf1(r,"<TR><TD>FlipCount: </TD><TD>%d</TD></TR>",sheap->flipcount);
 		ngx_http_authz_oidc_rprintf1(r,"<TR><TD>Cursor byte: </TD><TD>%d</TD></TR>",shdata_cursor(sheap));
 		
 		ngx_http_authz_oidc_rprintf1(r,"<TR><TD>Local Seg: </TD><TD>%d</TD></TR>",sheap->local_segment);
 		ngx_http_authz_oidc_rprintf1(r,"<TR><TD>apr_shm_t* valid: </TD><TD>%s</TD></TR>",sheap->shm_main==NULL?"FALSE":"TRUE");
 		ngx_http_authz_oidc_rprintf(r,"<TR><TD colspan='2'>&nbsp;<TD></TR>");
 		if(sheap->page!=NULL){
 			pg=sheap->page;
 			ngx_http_authz_oidc_rprintf(r,"\n<TR><TD colspan='2'><b>------Shared Portion--|</b></TD></TR>");
 			ngx_http_authz_oidc_rprintf(r,"\n<TR><TD colspan='2'>");
 			
 			ngx_http_authz_oidc_rprintf(r,"<TABLE>");
 			ngx_http_authz_oidc_rprintf1(r,"\t<TR><TD width='5'>&nbsp;</TD><TD>TimeStamp: </TD><TD>%s</TD></TR>",ctime(&(pg->timestamp)));
 			ngx_http_authz_oidc_rprintf1(r,"\t<TR><TD width='5'>&nbsp;</TD><TD>SegmentSize: </TD><TD>%d</TD></TR>",pg->segmentsize);
 			ngx_http_authz_oidc_rprintf1(r,"\t<TR><TD width='5'>&nbsp;</TD><TD>FlipCount: </TD><TD>%d</TD></TR>",pg->flipcount);
 			ngx_http_authz_oidc_rprintf1(r,"\t<TR><TD width='5'>&nbsp;</TD><TD>ItemsMax: </TD><TD>%d</TD></TR>",pg->itemmax);
 			ngx_http_authz_oidc_rprintf2(r,"\t<TR><TD width='5'>&nbsp;</TD><TD>Front/Back Segment: <TD>%d/%d</TD></TR>",pg->frontsegment,pg->backsegment);
 			seg=&(sheap->page->segments[sheap->page->frontsegment]);
 			
 			ngx_http_authz_oidc_rprintf1(r,"\t<TR><TD width='5'>&nbsp;</TD><TD colspan='2'>* Items(%d)---</TD></TR>",seg->itemcount);
 			ngx_http_authz_oidc_rprintf(r,"\t<TR><TD width='5'>&nbsp;</TD><TD colspan='2'>");
 				if(seg->itemcount>0){
 				ngx_http_authz_oidc_rprintf(r,"\t\t<TABLE>");
 				for(x=0;x<seg->itemcount;x++){	
 				ngx_http_authz_oidc_rprintf1(r,"<TR><TD>ID: </TD><TD>%s</TD></TR>",seg->items[x].ITEMID);
 				ngx_http_authz_oidc_rprintf1(r,"<TR><TD>INFO: </TD><TD>%s</TD></TR>",seg->items[x].INFO);
 				ngx_http_authz_oidc_rprintf1(r,"<TR><TD>Offset: </TD><TD>%d</TD>",seg->items[x].offset);
 				ngx_http_authz_oidc_rprintf1(r,"<TR><TD>Size: </TD><TD>%d</TD>",seg->items[x].size);
 				}
 				ngx_http_authz_oidc_rprintf(r,"\t\t</TABLE>");
 				}
 			ngx_http_authz_oidc_rprintf(r,"\t</TD></TR>");
 ...
 int oidc_showHeaders(ngx_http_authz_oidc_request_t *r, Config* config){
	...
	ngx_http_authz_oidc_rputs("<HTML>\n",r);
	ngx_http_authz_oidc_rprintf(r,"<HEAD><TITLE>Headers</TITLE></HEAD>\n");
	ngx_http_authz_oidc_rputs("<BODY>\n",r);
 ...
 int oidc_headers(ngx_http_authz_oidc_request_t *r, Config *config) {
...
int oidc_config_core_status(ngx_http_authz_oidc_request_t *r, Config *config) {
 	...
 	ngx_http_authz_oidc_rputs("<HTML>\n",r);
 	ngx_http_authz_oidc_rprintf(r,"<HEAD><TITLE>Config Core Status</TITLE></HEAD>\n");
 	ngx_http_authz_oidc_rputs("<BODY>\n",r);
 	ngx_http_authz_oidc_rputs("<TABLE width='100%'>",r);
 	ngx_http_authz_oidc_rprintf1(r,"<tr><td>HomeDir: </td><td>%s</td></tr>",SAFESTR(config->homeDir));
 	ngx_http_authz_oidc_rprintf1(r,"<tr><td>ConfigFile: </td><td>%s</td></tr>",SAFESTR(config->oidcConfigFile));
 	ngx_http_authz_oidc_rprintf1(r,"<tr><td>LogFile: </td><td>%s</td></tr>",SAFESTR(config->logFile));
 	if(config->configCore!=NULL){
 		int i=0;
 		config_core* core=config->configCore;

 		ngx_http_authz_oidc_rprintf(r,"<tr><td colspan='2' align='left'><strong>Globals</strong></td></tr>");
 		ngx_http_authz_oidc_rprintf1(r,"<tr><td>HomeDirectory: </td><td>%s</td></tr>",SAFESTR(core->globals->homeDir));
 		ngx_http_authz_oidc_rprintf1(r,"<tr><td>PassPhrase: </td><td>%s</td></tr>", core->passPhrase ? "non null" : "NULL");

 		ngx_http_authz_oidc_rprintf1(r,"<tr><td>DisableProcessRecovery: <td>%s</td></tr>",BOOLTOSTR(core->disableProcessRecovery));
 		ngx_http_authz_oidc_rprintf1(r,"<tr><td>OIDCHeaderPrefix: <td>%s</td></tr>",SAFESTR(core->oidcHeaderPrefix));

 		ngx_http_authz_oidc_rprintf(r,"<tr><td colspan='2' align='left'><strong>AutoRefresh settings</strong></td></tr>");
 		ngx_http_authz_oidc_rprintf1(r,"<tr><td>RefreshWaitSeconds: <td>%d</td></tr>",core->refreshWaitSeconds);
 		
 		if(core->globals->resourceService!=NULL){
 	 		ngx_http_authz_oidc_rprintf(r,"<tr><td colspan='2' align='left'><strong>Resource Service</strong></td></tr>");
 			ngx_http_authz_oidc_rprintf1(r,"<tr><td>Url: </td><td>%s</td></tr>",SAFESTR(core->globals->resourceService->uri));
 			ngx_http_authz_oidc_rprintf1(r,"<tr><td>Timeout: </td><td>%d</td></tr>",core->globals->resourceService->timeoutSeconds);
 		}

 		oidc_section_SharedHeap("Config Core",r,config->configCore->sheap);
 	}else{
 		ngx_http_authz_oidc_rprintf(r,"<tr><td>ConfigCore: </td><td>NULL</td></tr>");
 	}
 	ngx_http_authz_oidc_rputs("</TABLE>",r);
 	ngx_http_authz_oidc_rputs("</BODY></HTML>",r);
 	return NGX_OK;
...
static ngx_int_t ngx_http_authz_oidc_subrequest_post_handler(ngx_http_request_t *r, void *data, ngx_int_t rc) {
    ngx_http_request_t          *pr;
    ngx_http_authz_oidc_subrequest_ctx_t* ctx;

    pr = r->parent;

    ctx = ngx_http_get_module_ctx(pr, ngx_http_authz_oidc_module);
    ctx->done = 1;
...
static int ngx_http_authz_oidc_set_id_token_header(ngx_http_request_t*	httpRequest){

	// process module context
	ngx_http_authz_oidc_subrequest_ctx_t* ctx = ngx_http_get_module_ctx(httpRequest, ngx_http_authz_oidc_module);
    if (ctx != NULL) {
        if (!ctx->done) {
            return NGX_AGAIN;
        }
            ...
            if(responseBody!=NULL) {
				...
				if(json!=NULL) {
					...
					if(idTokenObj!=NULL) {
						...
						if(id_token!=NULL) {
							ngx_http_authz_oidc_setHeader(httpRequest, "Authorization", apr_pstrcat(p, "Bearer ", id_token, NULL));
						}
					}
				}
            }
...
static ngx_int_t
ngx_http_authz_oidc_set_subrequest_post_body(ngx_http_request_t * r, ngx_http_request_t * sr, const char* requestBody, ngx_str_t  contentType) {
	...
	/* set the 'Content-type' header */
	ngx_int_t rc = ngx_http_authz_oidc_set_content_headers(sr, rb->buf ? ngx_buf_size(rb->buf) : 0, contentType);
	if (rc != NGX_OK) {
		return NGX_ERROR;
	}
...
static ngx_int_t ngx_http_authz_oidc_preAuthorize(ngx_http_authz_oidc_request_t* r, Config* config) {
    ...
    ngx_http_authz_oidc_subrequest_ctx_t* ctx = ngx_http_get_module_ctx(r->httpRequest, ngx_http_authz_oidc_module);
    if (ctx != NULL) {
		...
		if(relyingParty!=NULL) {
			...
			ngx_int_t rc = ngx_http_authz_oidc_create_post_subrequest(r->httpRequest, oidc_authz_subrequest_type_oauth_token, requestBody);
			if (rc != NGX_OK) {
				return rc;
			}
			return NGX_AGAIN;
...
static ngx_int_t
ngx_http_authz_oidc_create_post_subrequest(ngx_http_request_t * r, oidc_authz_sub_request_type sub_request_type, const char* requestBody) {
    ngx_http_post_subrequest_t *psr;
    ngx_http_request_t *sr;
    ngx_int_t rc;

    // create new context
    ngx_http_authz_oidc_subrequest_ctx_t* ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_authz_oidc_subrequest_ctx_t));
	if (ctx == NULL) {
		return NGX_ERROR;
	}

	ngx_http_set_ctx(r, ctx, ngx_http_authz_oidc_module);
    ...
    psr->handler = ngx_http_authz_oidc_subrequest_post_handler;
    psr->data = ctx;
    ...
    /* allocate a buffer for your response body */
    rc = ngx_http_authz_oidc_set_subrequest_post_body(r, sr, requestBody, sub_request_type.content_type);
    if (rc != NGX_OK) {
        return rc;
    }
